docker_name = publishing-php-fpm
docker_webserver = publishing-webserver
docker_mysql = publishing-mysql
docker_image = backend_php-fpm
docker_nodejs = publishing-nodejs
...
init_app: set_env key_generate
	@echo 'Create .env and set app key!'
...
key_generate: # env key generate
	@sudo docker exec -it $(docker_name) bash -c 'php artisan key:generate'
...
prod: #optimizations for production server todo add npm run production
